...
class PatchHistory {
  ...
  undo(patchId) {
    ...
    if (index === -1 || index >= this._stack.length) {
      return { success: false, filePath: null, oldContent: null, error: "Patch not found in history" };
    }
  ...
  _load() {
    if (!this._storageDir) return;
    try {
      const data = fs.readFileSync(`${this._storageDir}/patch-history.json`, "utf8");
