...
class TokenManager {
  ...
  checkLimits(estimatedUsage = {}) {
    const warnings = [];
    const estimatedTokens = (estimatedUsage.inputTokens || 0) + (estimatedUsage.outputTokens || 0);
    ...
    if (daily.tokens + estimatedTokens > this.limits.dailyTokens) {
      warnings.push(`Daily token limit approaching (${Math.round((daily.tokens / this.limits.dailyTokens) * 100)}% used)`);
    }
    if (monthly.tokens + estimatedTokens > this.limits.monthlyTokens) {
      warnings.push(`Monthly token limit approaching (${Math.round((monthly.tokens / this.limits.monthlyTokens) * 100)}% used)`);
    }
    if (daily.costUsd + estimatedCost > this.limits.dailyCostUsd) {
      warnings.push(`Daily cost limit approaching ($${(daily.costUsd + estimatedCost).toFixed(2)}/${this.limits.dailyCostUsd})`);
    }
    if (monthly.costUsd + estimatedCost > this.limits.monthlyCostUsd) {
      warnings.push(`Monthly cost limit approaching ($${(monthly.costUsd + estimatedCost).toFixed(2)}/${this.limits.monthlyCostUsd})`);
    }
   ...
   */
  _estimateCost(usage) {
    const pricing = MODEL_COSTS[usage.model] || MODEL_COSTS["claude-sonnet-4-20250514"];
