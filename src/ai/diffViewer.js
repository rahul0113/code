  ...
  for (const raw of rawLines) {
    ...
    if (raw.startsWith("--- ") || raw.startsWith("+++ ") || raw.startsWith("@@")) {
      if (raw.startsWith("@@")) {
        ...
        lines.push({ type: LINE_TYPES.HEADER, content: raw, oldLine: null, newLine: null });
      } else {
        lines.push({ type: LINE_TYPES.HEADER, content: raw, oldLine: null, newLine: null });
      }
      continue;
    ...
    // Context line
    if (raw.startsWith(" ")) {
      lines.push({ type: LINE_TYPES.CONTEXT, content: raw.substring(1), oldLine, newLine });
      oldLine++;
      newLine++;
      stats.unchanged++;
    } else if (raw.startsWith("-")) {
      lines.push({ type: LINE_TYPES.REMOVE, content: raw.substring(1), oldLine, newLine: null });
      oldLine++;
      stats.removed++;
    } else if (raw.startsWith("+")) {
      lines.push({ type: LINE_TYPES.ADD, content: raw.substring(1), oldLine: null, newLine });
      newLine++;
      stats.added++;
    } else if (raw.length > 0) {
      // Treat as context
      lines.push({ type: LINE_TYPES.CONTEXT, content: raw, oldLine: null, newLine: null });
    }
  }
...
export { parseDiff, createDiffViewer, createLineElement, LINE_TYPES };
